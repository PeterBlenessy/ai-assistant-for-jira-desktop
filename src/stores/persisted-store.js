...
export const usePersistedStore = defineStore("persisted-store", () => {
    ...
    const addImprovementCommentsToJira = ref(loadStateFromLocalStorage("addImprovementCommentsToJira") || false);
    const generateChildIssuesForIssueTypes = ref(loadStateFromLocalStorage("generateChildIssuesForIssueTypes") || []);
    const availableIssueTypes = ref(loadStateFromLocalStorage("availableIssueTypes") || []);
    ...
    watch(addImprovementCommentsToJira, (newValue) =>
        saveStateToLocalStorage("addImprovementCommentsToJira", newValue)
    );
    watch(availableIssueTypes, (newValue) =>
        saveStateToLocalStorage("availableIssueTypes", newValue)
    );

    watch(
        generateChildIssuesForIssueTypes, (newValue) => {
            saveStateToLocalStorage("generateChildIssuesForIssueTypes", newValue);
        },
        { deep: true }
    );
